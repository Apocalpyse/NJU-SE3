 ...
 */
public class getDataByCode extends HttpServlet {
	...
	public static void main(String[] args){
		...
		try {
			arrayList=kAndEMA.getDataByCode(new InputStockByCodeVO("1/5/16","5/30/17","000001"));
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		ArrayList<StockVO> arrayList = null;
		try {
			arrayList=kAndEMA.getDataByCode(new InputStockByCodeVO(start,end,code));
		} catch (Exception e) {
			e.printStackTrace();
		}
		synchronized(this){
