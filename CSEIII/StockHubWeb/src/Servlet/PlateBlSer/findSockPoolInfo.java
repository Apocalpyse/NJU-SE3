 ...
 */
public class findSockPoolInfo extends HttpServlet{
	private static final long serialVersionUID = 1L;
	private Plate plate;
	/**
	 * @see HttpServlet#HttpServlet()
	 */
	public findSockPoolInfo(){
		super();
		plate=new Plate();
	}
	/**
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			if(stockPool.equals("SHANGZHENG")){
				arrayList=plate.findSockPoolInfo(start,end,StockPool.SHANGZHENG);
			}
			else if(stockPool.equals("SHENZHENG")){
				arrayList=plate.findSockPoolInfo(start,end,StockPool.SHENZHENG);
			}
			else if(stockPool.equals("HUSHEN300")){
				arrayList=plate.findSockPoolInfo(start,end,StockPool.HUSHEN300);
			}
			else if(stockPool.equals("SMALLMIDDLEPLATE")){
				arrayList=plate.findSockPoolInfo(start,end,StockPool.SMALLMIDDLEPLATE);
			}
			else if(stockPool.equals("STARTUPPLATE")){
				arrayList=plate.findSockPoolInfo(start,end,StockPool.STARTUPPLATE);
			}
