 ...
 */
public class getPotentialList extends HttpServlet{
	private static final long serialVersionUID = 1L;
	private Plate plate;
	/**
	 * @see HttpServlet#HttpServlet()
	 */
	public getPotentialList(){
		super();
		plate=new Plate();
	}
	/**
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			if(stockPool.equals("SHANGZHENG")){
				arrayList=plate.getPotentialList(StockPool.SHANGZHENG,date);
			}
			else if(stockPool.equals("SHENZHENG")){
				arrayList=plate.getPotentialList(StockPool.SHENZHENG,date);
			}
			else if(stockPool.equals("HUSHEN300")){
				arrayList=plate.getPotentialList(StockPool.HUSHEN300,date);
			}
			else if(stockPool.equals("SMALLMIDDLEPLATE")){
				arrayList=plate.getPotentialList(StockPool.SMALLMIDDLEPLATE,date);
			}
			else if(stockPool.equals("STARTUPPLATE")){
				arrayList=plate.getPotentialList(StockPool.STARTUPPLATE,date);
			}
