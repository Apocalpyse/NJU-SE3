 ...
 */
public class getVolumeList extends HttpServlet{
	private static final long serialVersionUID = 1L;
	private Plate plate;
	/**
	 * @see HttpServlet#HttpServlet()
	 */
	public getVolumeList(){
		super();
		plate=new Plate();
	}
	/**
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			if(stockPool.equals("SHANGZHENG")){
				arrayList=plate.getVolumeList(StockPool.SHANGZHENG,date);
			}
			else if(stockPool.equals("SHENZHENG")){
				arrayList=plate.getVolumeList(StockPool.SHENZHENG,date);
			}
			else if(stockPool.equals("HUSHEN300")){
				arrayList=plate.getVolumeList(StockPool.HUSHEN300,date);
			}
			else if(stockPool.equals("SMALLMIDDLEPLATE")){
				arrayList=plate.getVolumeList(StockPool.SMALLMIDDLEPLATE,date);
			}
			else if(stockPool.equals("STARTUPPLATE")){
				arrayList=plate.getVolumeList(StockPool.STARTUPPLATE,date);
			}
