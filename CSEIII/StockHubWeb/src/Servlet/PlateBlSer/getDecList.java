 ...
 */
public class getDecList extends HttpServlet{
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			if(stockPool.equals("SHANGZHENG")){
				arrayList=plate.getDecList(StockPool.SHANGZHENG,date);
			}
			else if(stockPool.equals("SHENZHENG")){
				arrayList=plate.getDecList(StockPool.SHENZHENG,date);
			}
			else if(stockPool.equals("HUSHEN300")){
				arrayList=plate.getDecList(StockPool.HUSHEN300,date);
			}
			else if(stockPool.equals("SMALLMIDDLEPLATE")){
				arrayList=plate.getDecList(StockPool.SMALLMIDDLEPLATE,date);
			}
			else if(stockPool.equals("STARTUPPLATE")){
				arrayList=plate.getDecList(StockPool.STARTUPPLATE,date);
			}
